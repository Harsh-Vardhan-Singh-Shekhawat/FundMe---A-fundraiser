...
const CampaignDetails = () => {
  ...
  const {donate, getCampaigns, address, contract} = useStateContext();
  ...
  return (
    ...
    <div className="w-full flex md:flex-row flex-col mt-10 gap-[30px]">
      <div className="flex-1 flex-col">
        <img src={location.state.campaign.image} alt="campaign" className="w-full h-[410px] object-cover rounded-xl"/>
        <div className="relative w-full h-[5px] bg-[#3a3a43] mt-2">
          <div className="absolute h-full bg-[#4acd8d]" style={{ width: `${calculateBarPercentage(location.state.campaign.target, location.state.campaign.amountCollected)}%`, maxWidth: '100%'}}>
          </div>
        </div>
      </div>

      <div className="flex md:w-[150px] w-full flex-wrap justify-between gap-[30px]">
        <CountBox title="Days Left" value={remDays} />
        <CountBox title={`Raised of ${location.state.campaign.target} ETH`} value={location.state.campaign.amountCollected} />
        <CountBox title="Total Campaigns" value={donators.length} />
      </div>
    </div>
    ...
    <div className="mt-[60px] flex lg:flex-row flex-col gap-5">
        ...
        <div>
          <h4 className="font-epilogue font-semibold text-[18px] text-[#808191] uppercase">Story</h4>
            ...
            <div className="mt-[20px]">
              <p className="font-epilogue font-normal text-[16px] text-[#808191] leading-[26px] text-justify">{location.state.campaign.description}</p>
            </div>
        </div>
      ...
      <div className="flex-1">
        ...
        <div className="mt-[20px] flex flex-col p-4 bg-[#dadde0] rounded-[10px]">
